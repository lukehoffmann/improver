...
class TrainRainForestsCalibration():
    ...
    def __init__(self, lead_times, thresholds, output_path):
        ...
        self.config = {
            self.lead_time_key(l) : {
                self.threshold_key(t): {} for t in self.thresholds
    ...
    def process(
        ...
        for lead_time in self.lead_times:
            ...
            for threshold in self.thresholds:
                ...
                model = self._train_model(training_data, obs_column, train_columns, lead_time, threshold)
                # TODO: Save model to the path
                path = str(output_path / self.model_filename(lead_time, threshold, "txt"))
                self.config[lkey][tkey]['lightgbm_model'] = path

        return self.config


    def _train_model(self, training_data, obs_column, train_columns, lead_time, threshold):
        """Train a model for each threshold."""
        import lightgbm
...
class TrainRainForestsCalibrationTreelite(TrainRainForestsCalibrationLightGBM):
    ...
    def process(self, training_data, obs_column, train_columns):
        TrainRainForestsCalibrationLightGBM.process(self, training_data, obs_column, train_columns)
        ...
        for lead_time in self.lead_times:
            ...
            for threshold in self.thresholds:
                tkey = self.threshold_key(threshold)
                # TODO: Get lightgbm model from the path
                # lightgbm_model = self.config[lkey][tkey]['lightgbm_model']

                # TODO: Compile and save model to the path
                # treelite_model = self._compile_model(lightgbm_model, path)

                path = str(output_path / self.model_filename(lead_time, threshold, "so"))
                self.config[lkey][tkey]['treelite_model'] = path

        return self.config


    def _compile_model(self, lightgbm_model, path):
